...
internal struct LogMessageReceiver: FeatureMessageReceiver {
    ...
    func receive(message: FeatureMessage, from core: DatadogCoreProtocol) -> Bool {
        do {
            guard let log: LogMessage = try message.baggage(label: LoggingMessageKeys.log) else {
                return false
            }
...
internal struct CrashLogReceiver: FeatureMessageReceiver {
    ...
    func receive(message: FeatureMessage, from core: DatadogCoreProtocol) -> Bool {
        do {
            guard let crash: Crash = try message.baggage(label: LoggingMessageKeys.crash) else {
                return false
            }
