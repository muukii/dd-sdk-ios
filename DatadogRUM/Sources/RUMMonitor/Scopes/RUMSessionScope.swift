...
internal class RUMSessionScope: RUMScope, RUMContextProvider {
    ...
    private var state: RUMSessionState {
        didSet {
            try? dependencies.core?.send(message: .baggage(label: RUMBaggageKeys.sessionState, value: state))
        }
    }
    ...
    func process(command: RUMCommand, context: DatadogContext, writer: Writer) -> Bool {
        ...
        if (isActive || deactivating) && !hasActiveView {
            ...
            try? dependencies.core?.send(message: .baggage(label: RUMBaggageKeys.viewReset, value: true))
        }
